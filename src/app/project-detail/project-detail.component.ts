...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, LayoutComponent, ReactiveFormsModule, FormsModule, FilterPipe, TitleCasePipe],
...
export class ProjectDetailComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    document.querySelectorAll('.toast-notification, .saving-message').forEach(el => el.remove());
  }

  loadProjectDetails(): void {
    this.isLoading = true;
    this.error = null; // Reset error
    this.projectSubscription = this.kanbanService.getKanbanById(this.projectId).subscribe({
      next: (project) => {
  ...
  loadTasks(): void {
    ...
    this.isLoading = true;
    this.tasksSubscription = this.kanbanService.getTasksByKanbanId(this.projectId).subscribe({
      next: (tasks) => {
  ...
  openTaskForm(): void {
    ...
    this.taskForm.reset({
      ...
      color: '#6366f1',
      title: '', description: '', deadline: null, assigneeEmail: null, storyPoints: null
    });
    this.assigneeNotFoundError = null;
  ...
  saveTask(): void {
    ...
    const taskPayload: Task = {
      ...(this.isEditingTask && this.currentTask?.id && { id: this.currentTask.id }),
    ...
    if (this.isEditingTask && this.currentTask?.id) {
      this.saveTaskSubscription = this.kanbanService.updateTask(this.currentTask.id, taskPayload, assigneeEmail).subscribe({
        next: (updatedTask: Task) => this.handleTaskSaveSuccess(updatedTask, 'Tâche mise à jour avec succès'),
        error: (err) => this.handleTaskSaveError(err)
      });
    } else {
      this.saveTaskSubscription = this.kanbanService.createTask(this.projectId, taskPayload, assigneeEmail).subscribe({
        next: (newTask: Task) => this.handleTaskSaveSuccess(newTask, 'Tâche créée avec succès'),
        error: (err) => this.handleTaskSaveError(err)
      });
    }
  }
  ...
  private normalizeTaskData(taskData: any): void {
    if (typeof taskData.status === 'string') {
      taskData.status = TaskStatus[taskData.status.toUpperCase() as keyof typeof TaskStatus] || TaskStatus.TODO;
    ...
    if (typeof taskData.priority === 'string') {
      taskData.priority = TaskPriority[taskData.priority.toUpperCase() as keyof typeof TaskPriority] || TaskPriority.MEDIUM;
  ...
  private showToastNotification(message: string, type: 'success' | 'error'): void {
    document.querySelectorAll(`.toast-notification.${type}`).forEach(el => el.remove());
